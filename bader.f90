...
program bader
    ...
    mol_grid_batches: do ib = 1,nbatch
        ...
        !
        if (new_atom) then
            if (sqrt(sum((xyzq(1:3,iat) - xyz(:,1))**2)) > 0.18) then
                ! GTOs don't have a cusp, need a minimum trust radius
                ptrust = .true.
                new_atom = .false.
            end if
        end if
...
function assign_atom(xyzatm, rad, natm, xyzw, npt, ndum, rho, drho)
    ...
    !
    logical     :: adone(npt)
    integer(ik) :: ipt, iat, iter, max_iter=5000
    ...
    propagate_density: do iter = 1,max_iter
        assign_points: do ipt = 1,npt
            if (.not.adone(ipt)) then
                ...
                delta = min(0.99*minval(dist), 0.2)
